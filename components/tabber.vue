      ...
      <view class="uni-tabbar">
         <view class="uni-tabbar__item" v-for="(item,index) in tabbar" :key="index" @tap="changeTab(item)">
            <view class="uni-tabbar__label" :class="{'active': item.pagePath == pagePath}">
               <image v-if="item.pagePath == pagePath && pagePath == '/pageIndex/index/index' && item.text !== 'add'"
                  src="@/static/fileBlue.svg" style="width:52rpx;height:52rpx" mode=""></image>
               <image v-else-if="item.pagePath == pagePath && pagePath == '/pageMy/my/my' && item.text !== 'add'"
                  src="@/static/iconBlue.svg" style="width:52rpx;height:52rpx" mode=""></image>
               <template v-else class="">
                  <view class="addClass" v-if="item.text == 'add'">
                     <image src="@/static/addFile.svg" style="width:52rpx;height:52rpx;margin-top:14rpx;" mode="">
                     </image>
                  </view>
         ...
         <filePop ref='midPop'>
            <view class="addFiles">
               <view class="titleFile">
                  ...
                  <view class="titleStl" @tap="handleNavigate('sendFile')">
                     <image class="wh30" src="@/static/sendFile.svg" mode=""></image>
                     <text class="mtop5">发文件</text>
                  </view>
                  <!-- </uni-file-picker> -->
                  <view class="titleStl" @tap="handleNavigate('collectFile')">
      ...
      methods: {
         ...
         handleNavigate(type) {
            switch (type) {
               ...
               case 'collectFile':
                  uni.navigateTo({
                     url: '/pages/collectFile/collectFile'
                  })
               default:
                  break;
            }
