  ...
  <view>
    <view v-if="showPage">
      <filePop ref='midPop' @maskClose="maskClose" @close="closePop">
        <view class="cloudPop">
          <view class="top">
            <image v-if="listItem.fileClass == 'image'" class="typeImg" src="@/static/type_3.svg"
              style="width:80rpx;height:80rpx;" mode=""></image>
            <image v-if="listItem.fileClass == 'video'" class="typeImg" src="@/static/type_2.svg"
              style="width:80rpx;height:80rpx;" mode=""></image>
            <image v-if="listItem.fileClass == 'file'" class="typeImg" src="@/static/type_4.svg"
              style="width:80rpx;height:80rpx;" mode=""></image>
            <image v-if="listItem.fileClass == 'floder'" class="typeImg" src="@/static/type_1.svg"
              style="width:80rpx;height:80rpx;" mode=""></image>
            <view class="msg_box">
          ...
          <view class="downCss pop-button mtop20" @click="downloadFile">
            下载文件
          </view>
  ...
  export default {
    ...
    methods: {
      async downloadFile(fileId) {
        try {
          const result = await createBoxDownLink({
            downFileVOS: [],
            downFolderVOS: [{
              folderId: this.listItem.folderId,
              absolutePath: this.listItem.folderName + '/'
            }]
          })
          const serializeResult = JSON.parse(JSON.stringify(result))
          if (!(serializeResult.data.code)) {
            uni.downloadFile({
              url: serializeResult.data.data,
              success: (res) => {
